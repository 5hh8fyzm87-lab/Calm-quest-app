...
**Status: OWNER GO-AHEAD RECORDED (Sep 2026).** The owner ordered build 14 to ship alongside their part-2 copy approval; these UI strings follow the owner-approved two-paths proposal and were reviewed in part-1 PR #20. Any label the owner wants changed once on device is a small follow-up edit — nothing below is re-opened for build 14.
...
**Status: OWNER-APPROVED AS WRITTEN (Sep 2026).** The owner reviewed the part-2 manifest in a single pass and approved every string exactly as written; it ships verbatim. The build-14 UI section above ships under the same go-ahead, and the build-13 section above that stays closed.
...
**Release note (build 14 submission):** with part 2 owner-approved and complete, `CONTENT_META.version` moved `1.1.0` → `1.2.0` — 1.1.0 was the state where the two newer programs shipped part-1 seed sets awaiting part 2, and that caveat no longer applies. No rendered string, pool length or schema changed with the bump; every string in this manifest still ships exactly as approved.
